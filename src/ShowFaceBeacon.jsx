...
.holdring circle{ fill:none; stroke-width:4; }
...
.fillcircle{ position:absolute; left:50%; top:50%; transform:translate(-50%,-50%);
  border-radius:50%; background:var(--ink); pointer-events:none; z-index:0; }
.beacon .core > *{ position:relative; z-index:1; }
.beacon .core{ overflow:hidden; }
.beacon.lit .core .label, .beacon.lit .core .meta, .beacon.lit .core svg{ color:var(--bg); }
        ...
        {tab === "out" && (
          <div className="scroll">
            ...
            <div className="stage" style={{ "--breath": `${breath}s`, "--pulse": `${pulseSpeed}s` }}>
              <button className={`beacon ${beacon ? "live" : "idle"} ${hold > 0 ? "holding" : ""} ${hold > 0.55 ? "lit" : ""}`}
                onClick={() => { if (beacon) tapBeacon(); }}
                onPointerDown={e => { e.preventDefault(); e.currentTarget.setPointerCapture?.(e.pointerId); beginHold(); }}
                ...
                <div className="core">
                  {hold > 0 && !beacon && (
                    <div className="fillcircle" style={{ width: `${hold * 118}%`, height: `${hold * 118}%` }} />
                  )}
